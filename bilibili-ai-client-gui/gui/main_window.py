...
class MainWindow:
    ...
    def _setup_messages_tab(self, parent):
        ...
        ttk.Button(btn_frame, text="重试失败", command=self._retry_failed).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="查看失败", command=self._show_failed).pack(side=tk.LEFT, padx=2)
    ...
    def _setup_settings_tab(self, parent):
        ...
        self.auth_entry = tk.Text(auth_frame, height=4, width=50, font=("Consolas", 9))
        self.auth_entry.pack(side=tk.LEFT, fill=tk.X, expand=True)
        bili_auth = config.get("bili_auth", "")
        display_text = "已登录（加密存储）" if bili_auth else "未登录"
    ...
    def _reprocess_blocked(self, uid=None):
        try:
            ...
            if blocked:
                ...
                main.reprocess_blocked_messages(uid=uid)
                self._refresh_messages()
    ...
    def _process_message(self):
        ...
        def do_process():
            try:
                ...
                extractor = subtitle_extractor.__class__(status_callback=on_status)
                subtitle_text = extractor.extract_text(f"https://www.bilibili.com/video/{bv_id}")
                ...
                if subtitle_text:
                    ...
                    if success:
                        self.root.after(0, self._set_status, f"已触发: {bv_id}")
                    else:
                        self.root.after(0, self._set_status, f"触发失败: {bv_id}")
                else:
                    self.root.after(0, self._set_status, f"无字幕: {bv_id}")
            except Exception as e:
    ...
    def _show_failed(self):
        ...
        for msg in failed:
            listbox.insert(tk.END, f"[{msg['bv_id']}] {msg.get('sender_name', msg['sender_uid'])}: {msg.get('content', '')[:40]}")

        def do_retry():
            selection = listbox.curselection()
            if not selection:
                return
            msg = failed[selection[0]]
            database.reset_message_status(msg["id"], "pending")
            self._refresh_messages()
            self._set_status(f"已重置: {msg['bv_id']}")
            dialog.destroy()

        btn_frame = ttk.Frame(dialog)
        btn_frame.pack(fill=tk.X, pady=5)
        ttk.Button(btn_frame, text="重试选中", command=do_retry).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="重试全部", command=lambda: [database.reset_message_status(m["id"], "pending") for m in failed] or [self._refresh_messages(), dialog.destroy(), self._set_status(f"已重置 {len(failed)} 条")]).pack(side=tk.LEFT, padx=2)
    ...
    def _retry_failed(self):
        ...
        for msg in failed:
            database.reset_message_status(msg["id"], "pending")
        self._refresh_messages()
        self._set_status(f"已重置 {len(failed)} 条失败消息")
    ...
    def _show_cookie_input(self):
        ...
        def save():
            ...
            if not cookie or cookie == "********":
                messagebox.showwarning("警告", "请输入有效的Cookie")
                return
            config.set("bili_auth", cookie)
            self.auth_entry.config(state="normal")
            self.auth_entry.delete("1.0", tk.END)
            self.auth_entry.insert("1.0", "已登录（加密存储）")
            self.auth_entry.config(state="disabled")
