...
def transcribe_audio(
    ...
    chunks: List[str] = []
    total = info.duration if info and info.duration else 0
    for idx, seg in enumerate(segments, start=1):
        text = seg.text.strip()
        if text:
            chunks.append(text)
        if total > 0 and idx % 20 == 0:
            progress = min(100, int(seg.end / total * 100))
            log(f"转写中：{progress}% ({idx} 片段, {seg.end:.0f}s/{total:.0f}s)...")
...
class SubtitleExtractor:
    def __init__(self, cookie: Optional[str] = None, proxy: Optional[str] = None, status_callback: Optional[Callable[[str], None]] = None):
        from utils.app_data import APP_DATA_DIR
        if cookie is None:
