...
def login_status():
    logger.info(f"/login_status called, cookie in LOGIN_DATA: {bool(LOGIN_DATA.get('cookie'))}")
    if LOGIN_DATA.get("cookie"):
    ...
    if result.get("status") == "confirmed" and LOGIN_DATA.get("cookie"):
